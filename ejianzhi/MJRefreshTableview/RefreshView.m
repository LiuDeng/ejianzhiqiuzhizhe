...
- (id)initWithFrame:(CGRect)frame atTop:(BOOL)top
{
    ...
    if (self)
    {
        ...
        self.textLabel.text = top ? @"下拉刷新" : @"没有了哦";//上拉加载更多
...
- (void)layouts
{
    ...
    if (self.isAtTop)
    {
        ...
        y = y - kPRLabelHeight;
        stateFrame = CGRectMake(0, y, size.width-5, kPRLabelHeight);
...
//        self.arrowLayer.contents = (id)arrow.CGImage;
        self.textLabel.text = @"加载中...";//释放加载更多//
      
        
    }
    
    _textLabel.textColor=COLOR(130, 130, 130);
    self.dateLabel.textColor = [UIColor grayColor];
    self.textLabel.frame          = stateFrame;
    self.dateLabel.frame          = dateFrame;
    self.arrowLayer.frame         = arrowFrame;
    self.activityView.frame       = CGRectMake((SCREENWIDTH-20)/2+35, y-5, 20, 20);
