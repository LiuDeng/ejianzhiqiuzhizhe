...
- (void)animateTableViewShow:(BOOL)show complete:(void(^)())complete {
    if (show) {
        ...
        _leftTableView.frame = CGRectMake(self.origin.x, self.frame.origin.y, ScreenWidth/3, 0);
        [self.superview addSubview:_leftTableView];
        _rightTableView.frame = CGRectMake(self.origin.x+ScreenWidth/3, self.frame.origin.y, ScreenWidth/3*2, 0);
        [self.superview addSubview:_rightTableView];
        
        _leftTableView.alpha = 1.f;
        _rightTableView.alpha = 1.f;
        [UIView animateWithDuration:0.2 animations:^{
            _leftTableView.frame = CGRectMake(self.origin.x, self.frame.origin.y, ScreenWidth/3, _height);
            _rightTableView.frame = CGRectMake(self.origin.x+ScreenWidth/3, self.frame.origin.y, ScreenWidth/3*2, _height);
